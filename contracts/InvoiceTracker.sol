...
contract InvoiceTracker is Owned {
  ...
  /// @dev no other details.
  function isNoClient(string memory _clientName, address _clientID) private view returns(bool) {
  ...
  function addClient(address _clientID, string memory _name) noDupClient(_clientID, _name) public {
  ...
  /// @dev no other details.
  function isNoDuplicateInvoice(string memory _clientName, uint256 _invoiceNumber) private view returns(bool) {
  ...
  modifier noDupInvoice(string memory _clientName, uint256 _invoiceNumber) {
      require(isNoDuplicateInvoice(_clientName, _invoiceNumber), "Duplicate invoice");
      _;
  }
  ...
  function addInvoice(
            ...
            uint256 _due90DaysDate,
            uint256 _due120DaysDate,
            uint256 _datePmtReceived) noDupInvoice(_clientName, _invoiceSentDate) public {
